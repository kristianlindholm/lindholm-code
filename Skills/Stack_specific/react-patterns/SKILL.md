...
- **Turbopack is the default dev bundler** from Next.js 16 (`next dev`). Disable with `--webpack` only to work around a Turbopack bug or a webpack-only dev plugin.
- **Middleware lives in `proxy.ts`** (project root) from Next.js 16, replacing `middleware.ts`. This is correct and intentional — do NOT flag `proxy.ts` as misnamed or suggest renaming it to `middleware.ts`; the rename is tied to the Next.js version, and renaming breaks middleware execution. Pre-16 projects still use `middleware.ts`.
