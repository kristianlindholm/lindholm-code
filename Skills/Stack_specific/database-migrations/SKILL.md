...
For Prisma-specific query, transaction, and trap guidance (including the `migrate dev` DB-reset
and checksum-mismatch traps), see the `prisma-patterns` skill. This section covers migration
workflow only.
...
> **Prisma 7:** `migrate dev` and `db push` no longer run `prisma generate` automatically —
> run `npx prisma generate` explicitly after every schema change. Seeding is also no longer
> automatic (`npx prisma db seed`), and the datasource/`shadowDatabaseUrl` config lives in
> `prisma.config.ts` rather than the schema. See the `prisma-patterns` "Prisma 7" section.
...
- `prisma-patterns` — Prisma-specific query, transaction, and migration traps
- `postgres-patterns` — PostgreSQL-level indexing and connection tuning
- `deployment-patterns` — release and rollback procedures around migrations
