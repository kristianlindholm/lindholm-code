...
## Route Handler Integration Tests (Next.js)

App Router route handlers are plain functions that take a `Request`/`NextRequest` and return a `Response`. Test them by importing the exported HTTP method and invoking it directly — no running server needed.

```tsx
import { NextRequest } from "next/server";
import { GET } from "./route";

test("GET /api/users returns the user list", async () => {
...
test("GET /api/users rejects an invalid limit with 400", async () => {
  const request = new NextRequest("http://localhost/api/users?limit=abc");
...
- Assert on `response.status` and the parsed body — the handler's observable contract — not on which internal functions were called.
